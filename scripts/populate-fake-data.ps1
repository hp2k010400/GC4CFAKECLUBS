...
# Map product type string -> group key
function Get-TypeGroup($productType) {
  switch -Wildcard ($productType.ToLower()) {
...
$brandData = @{
  ...
  "Callaway" = @{
    "driver" = @{
      fakeIndicators = @(
        "Hold a magnet to the clubhead — genuine titanium and carbon composite heads will not attract a magnet; most counterfeits use cheaper ferrous metal that sticks",
        "Tap the carbon composite crown with a fingernail — genuine carbon produces a dull soft thud; a metallic ringing ping means it is fake steel painted to look like carbon",
      ...
      authenticityNotes = "Callaway maintain a counterfeit warning page at callawaygolf.com/help/counterfeit-warning and will verify clubs by serial number. The magnet test is one of the quickest checks for titanium/carbon drivers."
    ...
    "iron" = @{
      fakeIndicators = @(
        ...
        "Forged Apex irons produce a distinctively soft, dampened feel at impact — fakes feel noticeably harder and produce a sharper, more metallic sound",
      ...
      authenticityNotes = "Callaway Apex and Rogue irons are commonly counterfeited. The Face Cup technology in cavity-back models produces a higher-pitched sound on off-centre hits that cast fakes cannot replicate. Verify serial numbers via Callaway customer service."
...
$modelOverrides = @(
  ...
  @{ brand="Mizuno"; match="jpx"; fakeIndicators=@("Grain Flow Forged stamp on forged JPX models precisely finely etched — fakes shallow or blurred","Inside hosel: genuine has clear adhesive-aid grooves; fakes smooth","Serial laser-etched on hosel above ferrule in JP + year + sequence format — fakes wrong format or no serial","Head weight progression 7-8g per iron — fakes show no logical progression","Mizuno logo in cavity slightly recessed with crisp edges — fakes flush or raised") }
...
foreach ($model in $models) {
  ...
  $typeGroup = Get-TypeGroup ($model.productType ?? "")
